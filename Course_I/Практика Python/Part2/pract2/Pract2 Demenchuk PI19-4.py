...
X_LIST = [
    ...
    for x in range(
        X_INTERVALS_LIST[index] * 10, int(X_INTERVALS_LIST[index + 1] * 10), int(H * 10)
    )
]
Y_LIST = [
    FUNCTION_LIST[index](x / 10)
    for index in range(3)
    for x in range(
        X_INTERVALS_LIST[index] * 10, int(X_INTERVALS_LIST[index + 1] * 10), int(H * 10)
    )
...
fig, (ax1, ax2, ax3) = plt.subplots(
    nrows=3,
    ncols=1,  # объявление обекта Figure (переменная fig)
...
for index in range(3):
    opr_integ = INTEGRATED_FUNCTION_LIST[index](
        X_INTERVALS_LIST[index + 1]
    ) - FUNCTION_LIST[index](X_INTERVALS_LIST[index])
    mat_o += opr_integ

ax1.plot(X_LIST, Y_LIST, "r")  # работа с первым графиком(ax1)
ax1.set_title(f"M(x) = {mat_o}")  # изображен просто график кастомной функции
lgnd1 = ax1.legend(
    ["Custom function"], loc="upper center", shadow=True
)  # как в варианте
...
y_trend = [
    linear_function(x, a1, b1) for x in new_X_LIST
]  # создаем список значений у для линейной функции типа a + bx
R_1 = get_r_2(
    X_LIST, Y_LIST, [a1, b1], func=linear_function
)  # получаем коэф. детерминации и среднее отклонение
otkl_x1, otkl_y1 = get_sr_otkl(X_LIST, y_trend)
ax2.plot(
    X_LIST, Y_LIST, "r", new_X_LIST, y_trend, "b", linestyle="solid"
)  # рисуем график (даже два), "r" и "b" это цвета если чо
ax2.set_title(
    f"R^2 = {R_1}\nS(x) = {otkl_x1}\nS(y) = {otkl_y1}"
)  # делаем заголовочек @-@
lgnd2 = ax2.legend(
    ["Custom function", f"y = {a1} + {b1}x"], loc="upper center", shadow=True
)
lgnd2.get_frame().set_facecolor("#ffb19a")

a2, b2, c2 = get_koef_polin(X_LIST, Y_LIST)  # тут впринципе то же самое, только
y_trend2 = [
    polim_function(x, a2, b2, c2) for x in new_X_LIST
]  # системка из 3х уравнений
R_2 = get_r_2(
    X_LIST, Y_LIST, [a2, b2, c2], func=polim_function
)  # а не из двух как в линейной функции
...
lgnd3 = ax3.legend(
    ["Custom function", f"y = {a2}x^2 + {b2}x + {c2}"], loc="upper center", shadow=True
)
