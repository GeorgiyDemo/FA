...
def main():
    ...
    for _ in range(aircraft_count):
        ...
        aircraftclass_dictargs = {
            1: [fake.word(), randint(1000, 200000), data_module.plane_type(), randint(1, 10000),
                data_module.get_country(), data_module.get_country(), randint(300, 2500), "ракеты", randint(500, 10000),
                randint(1000, 100000), 100000, 10],
            2: [fake.word(), randint(1000, 200000), data_module.helicopter_type(), randint(1, 10000),
                data_module.get_country(), data_module.get_country(), randint(2, 8), randint(20, 40),
                data_module.get_country()],
        }
        if r_num == 1:
            plane_obj_list.append(aircraftclass_dict[r_num](*aircraftclass_dictargs[r_num]))
        else:
            helicopter_obj_list.append(aircraftclass_dict[r_num](*aircraftclass_dictargs[r_num]))

    print("Хорошо, я сгенерировал {} самолетов и {} вертолетов, давайте взгляним на них?".format(len(plane_obj_list),
                                                                                                 len(
                                                                                                     helicopter_obj_list)))
    input()
    ...
    for _ in range(pvo_count):
        ...
        pvo_dictargs = {
            1: [fake.word(), randint(1000, 200000), randint(2, 8), randint(16, 128), randint(1000, 200000),
                randint(1000, 200000), choice(["стационарное", "перемещаемое"]), randint(5, 80)],
    ...
    print("Хорошо, я сгенерировал {} ракетных и {} зенитных вида ПВО, заценим?".format(len(missile_list_obj),
                                                                                       len(antiaircraft_list_obj)))
    input()
    ...
    if result == True:
        print("Мы уничтожили объект с названием '{}' и типом {}".format(killedobj_list[obj_number].name,
                                                                        killedobj_list[obj_number].type))
