...
def main():
    try:
        ...
        return
    d = {1: ClentClass, 2: ContributorClass, 3: CreditorClass, 4: OrganizationClass, }
    fake = Faker(['ru_RU'])
    bankclients_list = []
    for _ in range(n):
        r_int = randint(1, 4)
        d_args = {1: (fake.name(), fake.date(pattern='%d.%m.%Y'), randint(1, 1000000), randint(0, 100)),
                  2: (fake.name(), fake.date(pattern='%d.%m.%Y'), randint(1, 1000000), randint(0, 100)), 3: (
            fake.name(), fake.date(pattern='%d.%m.%Y'), randint(1, 1000000), randint(0, 100), randint(1, 1000000)),
