...
class MainClass:
    ...
    def _display_students(self):
        ...
        table_list.extend(
            [
                [
                    str(i + 1),
                    stud_obj_list[i].name,
                    stud_obj_list[i].group,
                    str(stud_obj_list[i].course),
                    str(round(stud_obj_list[i].points, 2)),
                    str(stud_obj_list[i].mark),
                ]
                for i in range(len(stud_obj_list))
            ]
        )
    ...
    def change_students(self):
        ...
        s_number = input(
            "Введите номер студента для редактирования информации о нём -> "
        )

        if UtilClass.is_digital(s_number) and int(s_number) in [
            i + 1 for i in range(0, len(stud_obj_list))
        ]:
    ...
    def set_students(self):
        ...
        work_count = int(
            input(
                "Введите количество работ за половину семестра (кол-во работ в 1 и 2 половине семестра равно) -> "
            )
        )
        ...
        for i in range(students_count):
            ...
            for j in range(2):
                ...
                cert_obj = CertificationClass(
                    "Промежуточная аттестация №{}".format(j + 1)
                )
                ...
                for k in range(work_count):
                    ...
                    while bool_flag:
                        try:
                            ...
                            w_type = input(
                                "Введите тип работы (практика/контрольная/тестирование) -> "
                            )
                            w_deadline = input(
                                "Введите дату дедлайна работы в формате ДД.ММ.ГГГГ (пример: 01.01.2020) -> "
                            )
                            w_completed = UtilClass.boolean_dict[
                                input("Работа завершена? (Да/Нет) -> ")
                            ]
                            ...
                            if w_completed:
                                ...
                                w_protected = UtilClass.boolean_dict[
                                    input("Работа защищена? (Да/Нет) -> ")
                                ]
                                if w_protected:
            ...
            while bool_flag:
                try:
                    s_exam = UtilClass.boolean_dict[
                        (input("{} сдавал экзамен? (Да/Нет) -> ".format(s_name)))
                    ]
                    if s_exam:
                        s_name = input("Введите полное название экзамена ->")
                        s_points = int(
                            input("Введите кол-во баллов на экзамене (макс. 60) -> ")
                        )
