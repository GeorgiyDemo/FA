...
class SQLiteClass:
    ...
    def get_students(self):
        ...
        for student_args in list(c.execute("SELECT student_id, name, groups, course FROM student")):
            ...
            for certification in list(
                    c.execute("SELECT cert_id, name FROM certification WHERE student_id=" + str(current_studentid))):
                ...
                for work in list(c.execute("SELECT * FROM work WHERE cert_id=" + str(current_certid))):
                    d = dict(zip(
                        ["cert_id", "student_id", "name", "points", "work_type", "date_deadline", "date_completed",
                         "date_protected", "work_completed", "work_protected"], work))
                    work_obj = WorkClass(d["name"], d["work_type"], d["date_deadline"], bool(d["work_completed"]),
                                         d["date_completed"], bool(d["work_protected"]), d["date_protected"])
    ...
    def set_students(self, s_obj):
        ...
        r = c.execute(
            "INSERT INTO student (name, groups, course, points, mark) VALUES ('{}','{}','{}',{},{})".format(s_obj.name,
                                                                                                            s_obj.group,
                                                                                                            s_obj.course,
                                                                                                            s_obj.points,
                                                                                                            s_obj.get_mark(
            ...
            for cert in s_obj.cert_obj_list:
                r = c.execute(
                    "INSERT INTO certification (student_id, name, points, date_begin, date_end) VALUES ({},'{}',{},'{}','{}')".format(
                        current_studentid, cert.name, cert.points, cert.date_begin.strftime("%d.%m.%Y"),
                ...
                for work in cert.work_obj_list:
                    ...
                    work_args_list = [current_certid, current_studentid, work.name, work.points, work.work_type,
                                      work.date_deadline.strftime("%d.%m.%Y"), date_completed, date_protected,
                                      work.work_completed, work.work_protected]
                    c.execute(
                        "INSERT INTO work VALUES ({},{},'{}',{},'{}','{}','{}','{}',{},{})".format(*work_args_list))
    ...
    def remove_students(self, s_obj):
        ...
        r = c.execute(
            "SELECT student_id FROM student WHERE (name='{}' AND groups='{}' AND course='{}')".format(s_obj.name,
                                                                                                      s_obj.group,
                                                                                                      s_obj.course))
