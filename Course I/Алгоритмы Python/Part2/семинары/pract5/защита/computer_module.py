...
class ComputerAnalyserClass(UserAnalyserClass):
    ...
    def __init__(self, command_dict, board_obj):
        ...
        print("ComputerAnalyserClass",self.results_list)
...
class ComputerGameClass:
    ...
    def processing(self):
        ...
        for field in myfields_arr:
            ...
            #print(field.figure_obj)
            print("Клетка: x{}, y{} Фигура: x{}, y{}, если преобразуем коорды фигуры: x{}, y{}".format(field.coord_x, field.coord_y, field.figure_obj.coord_x, field.figure_obj.coord_y, field.figure_obj.coord_x, UtilClass.xint2char(field.figure_obj.coord_y)))
            ...
            if self.board_obj.detect_element(y-1,x-1):
                ...
                all_d = np.append(all_d,{'from': {'x': x, 'y': y_char}, 'to': {'x': new_x, 'y': new_y}, 'mode': 'peace', 'user_color': reverse_uc})
            ...
            if self.board_obj.detect_element(y+2,x-2):
                ...
                all_d = np.append(all_d,{'from': {'x': x, 'y': y_char}, 'to': {'x': new_x, 'y': new_y}, 'mode': 'war', 'user_color': reverse_uc})
        ...
        for d in all_d:
            ...
            if obj.boolean_result:
                print("Все классно, мы походим")
                #TODO Осуществляем перемещение фигуры, изменение ее координат и ее перепривязка
                break
        else:
            self.result = False
            print("ВЫШЛИ БЕЗ БРЕЙКА")
