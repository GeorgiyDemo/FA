...
class AddTicketClass():
    ...
    def auto_components_reserve(self):
        ...
        print(
            "Зарезервировали " + place + " место в вагоне " + car + " по маршруту " + selected_train["from"] + " -> " +
            selected_train["to"] + " за " + str(price) + " руб")
        ...
        while exit_flag == False:
            ...
            for i in range(len(content)):
                if content[i]["from"] == self.way_from and content[i]["to"] == self.way_to:
                    ...
                    print("Всего в поезде " + str(content[i]["info"]["car_count"]) + " вагонов и " + str(
                        content[i]["info"]["places_free"]) + " свободных мест\nМест по вагонам:")
    ...
    def place_searcher(self, way, selected_car):
        ...
        while exit_flag == False:
            ...
            selected_place = input("Введите номер места для бронирования (0 для выхода из подменю) -> ")
            if selected_place == "0":
                exit_flag = True
            elif selected_place in buf_place_list:
                if content[way]["train"][selected_car]["cars"][selected_place]["name"] == None:
                    ...
                    if user_reply == "Да" or user_reply == "Y" or user_reply == "y":
                        ...
                        if question_input == "Да":
                            ...
                            if pay_obj.result == True:
                                ...
                                main_text_str_ticket = "\nБилет\nСтанция и время отправления: " + content[way][
                                    "from"] + " " + content[way]["time_begin"] + "\n" + "Место и время прибытия: " + \
                                                       content[way]["to"] + " " + content[way][
                                                           "time_finish"] + "\nМесто: №" + selected_place + " вагон " + selected_car + " [" + \
                                                       buf_place["type"] + "]"
                            ...
                            print("Хорошо, оплатить билет вы можете позже в пункте 'Управление моими билетами'")
...
class RemoveTicketClass():
    ...
    def ticket_remover(self):
        ...
        refound_str = "Вам вернется " + str(refound_price) + " руб. Сервис удержит комиссию в виде " + str(
            percent_price) + " руб."
        ...
        if confirm_input == "Да" or confirm_input == "Y" or confirm_input == "y":
            ...
            main_text_str_name = "ФИО клиента: " + self.name
            main_text_str_ticket = "\nБилет\nСтанция и время отправления: " + content[way]["from"] + " " + content[way][
                "time_begin"] + "\n" + "Место и время прибытия: " + content[way]["to"] + " " + content[way][
                                       "time_finish"] + "\nМесто: №" + place + " вагон " + car + " [" + selected_place[
                                       "type"] + "]"
            main_text_str_payment = "\nВозврат средств\nСтоимость билета: " + str(
                selected_place["price"]) + " руб. \n" + "Стоимость комиссии: " + str(
